...
v_dsas = 0.305e-3; % Sample thickness for pType
% v_dsas = 446.02E-6; %
...
%             initial result @ 1 THz
        ...
        for lv_i = 1:length(a_inds)
            a_sols(lv_i) = a_vx(a_cols(lv_i)) + 1i*a_vy(a_rows(lv_i));
%             a_checksols(lv_i) = abs((a_sols(lv_i))/(real(a_sol(lv_k-1)))); %Here I put this to choose the value closer to the previous obtained
            a_checksols(lv_i) = abs(abs(a_sols(lv_i))/abs(a_z0(lv_k)));
            a_checksolsb(lv_i,:) = [a_sols(lv_i) a_checksols(lv_i)];
        end
    end
...
%         lv_f = lv_f + 1;
%         figure(lv_f)
%         surfc(x,y,log10(m_diff),'EdgeColor','none')
%         view(2)
%         colorbar('southoutside');
...
m_ereal = real(a_diel);
m_eimag = imag(a_diel);
m_saver = [a_freq; m_rreal; m_rimag; m_ereal; m_eimag];
newfilename = sprintf('%s%s',v_directory,f_namedata,'_TDSresults', '.dat');
fid = fopen(newfilename, 'wt');
%Header
fprintf(fid,'Frequency(Hz) Real_refrac Imag_refrac Real_diel Imag_diel\n')
%Body
